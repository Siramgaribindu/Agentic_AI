...
if not st.session_state.interview_started:
    ...
    if st.button("🚀 Start Interview", use_container_width=True, type="primary"):
        if not role.strip():
            ...
            try:
                response = requests.post(f"{BACKEND_URL}/api/interview/next-question", json=payload)
                data = response.json()
...
# ---------- INTERVIEW SCREEN ----------
    ...
    # Find last assistant message index for speech synthesis
    ...
    for idx, msg in enumerate(st.session_state.messages):
        if msg["role"] == "assistant" and "## SCORECARD" in msg["content"]:
            ...
            # Trigger TTS for the latest question only
    ...
    if st.session_state.current_stage == "FEEDBACK":
        if len(st.session_state.messages) > 0 and "## SCORECARD" not in st.session_state.messages[-1]["content"]:
            payload = {
                "role_name": st.session_state.job_role,
            ...
            with st.spinner("Generating Interview Scorecard..."):
                try:
                    response = requests.post(f"{BACKEND_URL}/api/interview/generate-scorecard", json=payload)
                    data = response.json()
